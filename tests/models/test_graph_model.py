...
def generate_mixed_batch():
    batch = Batch.from_data_list([generate_sample_data() for _ in range(6)] + [generate_small_sample_data() for _ in range(3)] )
    return batch


def test_success_model():
    model = YomiSuccessModel(hidden_dim=8, num_layers=3, final_dim=5, num_heads=2, dropout=0.2, input_bias=True, bias=True)
    _ = model.eval()
...
def test_choice_model():
    model = YomiHandChoiceModel(hidden_dim=8, num_layers=3, final_dim=5, num_heads=2, dropout=0.2, input_bias=True, bias=True)
    _ = model.eval()
    batch = generate_mixed_batch()
    undirected_transformer = ToUndirected()
    batch = undirected_transformer(batch)
