...
class Arena(ABC):
    ...
    def __init__(self, left_strategy='best_play', 
        ...
        if left_generator:
            ...
            self.left_player = self.player_generator(side='left', strategy=left_strategy)

        if right_generator:
            self.right_player = right_generator(side='right', strategy=right_strategy)
        else:
            self.right_player = self.player_generator(side='right', strategy=right_strategy)
